...
import { cn } from "../lib";
import openFolderIcon from "/openFolder.png";
      ...
      {folder.childrens && folder.open && (
        <ul>
          {folder.childrens.map((subFolder) => {
            return (
              <li
                className={cn("pl-4 border-l border-l-gray-500 flex flex-row")}
                key={subFolder.title}
              >
  ...
  return (
    ...
    >
      {isFolder ? (
        ...
        <File className="w-full h-full" color="black" />
      )}
      <p className="pl-3 truncate max-w-full text-black" title={folder.title}>
        {folder.title}
      </p>
    </div>
  );
...
const IconView = () => {
  ...
  return (
    <div
      className="w-full h-full bg-neutral-300 rounded focus-visible:outline-none"
      tabIndex={-1}
  ...
  return (
    ...
    >
      <div className="flex items-center h-screen">
        <div className="w-96 h-full py-20 px-4 bg-[#161C2A]">
          <TreeView />
        </div>
        <div className="w-full h-full py-12 px-12 bg-[#0C0F18]">
          <IconView />
        </div>
      </div>
    </FileExplorerContext.Provider>
